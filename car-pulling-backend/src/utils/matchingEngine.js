...
const MIN_SHARED_HEXAGONS = 1; // Require only 1 shared hex cell (was 3, too strict)
...
function phase1HexOverlap(driverCoords, passengerCoords) {
  if (!h3 || driverCoords.length < 2 || passengerCoords.length < 2) {
    console.log('[PHASE-1] ⚠️  Skipping H3 check (h3 unavailable or insufficient data)');
    return true; // Fallback: continue to Phase 2 if H3 unavailable
  }
 ...
 * Increased from 100m - routes in same area may not directly overlap
 * but can still share a common segment for pickup/dropoff
 * @param {Array} driverCoords - Driver route [[lat, lng], ...]
 * @param {Array} passengerCoords - Passenger route [[lat, lng], ...]
...
function getSpatialOverlapSegment(driverCoords, passengerCoords) {
  try {
    ...
    console.log(`[OVERLAP] 📍 Checked ${passengerCoords.length} passenger points, found ${overlappingPoints.length} within buffer`);

    if (overlappingPoints.length < 1) {
      console.log('[OVERLAP] ❌ No overlapping points found');
      return null; // Need at least 1 point to match
    }

    console.log(`[OVERLAP] ✅ Overlapping points: ${overlappingPoints.map(p => `[${p[0].toFixed(5)}, ${p[1].toFixed(5)}]`).join(', ')}`);
...
function phase2PreciseMatch(driverCoords, passengerCoords) {
  ...
  if (!overlapPoints) {
    ...
    return {
      ...
      message: 'Routes do not overlap within 500m buffer'
    };
  }
